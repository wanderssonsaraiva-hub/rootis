...
function renderPatientPortal(){
  ...
  if(!days.length){root.innerHTML='<div class="patient-empty">No momento não há vagas disponíveis para agendamento. Novos dias aparecerão aqui assim que o dentista abrir horários.</div>';}
  else{
    root.innerHTML=days.map(({key,freeSlots})=>`<div class="public-day"><div class="public-day-head"><strong>${publicFormatDate(key)}</strong><span>${freeSlots.length} ${freeSlots.length===1?'vaga disponível':'vagas disponíveis'}</span></div><div class="public-slots">${freeSlots.map(slot=>`<button type="button" class="public-slot" data-public-date="${key}" data-public-start="${slot.start}" data-public-end="${slot.end}">Vaga ${slot.originalIndex+1} · ${slot.start} às ${slot.end}</button>`).join('')}</div></div>`).join('');
...
if(patientBookingForm){
  ...
  patientBookingForm.addEventListener('submit',e=>{
    ...
    e.target.reset();renderPatientPortal();publicSelected=null;document.getElementById('chosenSlot').textContent='Nenhuma vaga selecionada.';
  });
}
